 ...
 *
 * Source: mod-host src/lv2/lv2-hmi.h, as built for this device, i.e. MOD's
 *         header plus LoopPad_Jack's patch
 *         board/looppad_jack/patches/mod-host/0005-hmi-add-a-set_led_rgb-widget-call.patch
 *
 * There is no shared staging copy of this header -- it is not installed into
 * the Buildroot sysroot -- so mod-host and every plugin that talks to it each
 * carry their own. If this file and mod-host's disagree about the layout of
 * LV2_HMI_WidgetControl, calls go through the WRONG SLOT of the vtable. Diff
 * this file against mod-host's copy under
 * Buildroot/build/output/build/mod-host-<sha>/src/lv2/ after any mod-host
 * bump; only comments should differ.
 *
 * set_led_rgb and LV2_HMI_WIDGETCONTROL_SIZE_SET_LED_RGB are the LoopPad_Jack
 * addition; everything else is upstream MOD. Carrying the extended header is
 * safe on stock MOD firmware too, because every call to the extra slot is
 * guarded on `size >= LV2_HMI_WIDGETCONTROL_SIZE_SET_LED_RGB` -- a stock host
 * reports the smaller size and the plugin falls back to the palette call.
 *
 * ---------------------------------------------------------------------------
 *
...
/**
 * Size check for LV2_HMI_WidgetControl::set_led_rgb call.
 */
#define LV2_HMI_WIDGETCONTROL_SIZE_SET_LED_RGB (LV2_HMI_WIDGETCONTROL_SIZE_POPUP_MESSAGE + sizeof(void*))
...
typedef struct {
    ...
    /**
     * Set the LED to an arbitrary 8-bit RGB colour, with optional blink timing
     * in milliseconds from 0 to 5000, as for set_led_with_blink.
     *
     * This is a LoopPad_Jack addition, not part of MOD's own extension. It
     * exists because LV2_HMI_LED_Colour has eight entries and no brightness
     * dimension that can be combined with blinking, which is not enough for a
     * surface whose colour scheme carries two things at once -- state as hue
     * and selection as brightness.
     *
     * A host that does not offer it reports a smaller @a size, and the plugin
     * is expected to fall back to set_led_with_blink with whichever palette
     * entry is nearest.
     *
     * @note This function must only be used if size >= LV2_HMI_WIDGETCONTROL_SIZE_SET_LED_RGB
     */
    void (*set_led_rgb)(LV2_HMI_WidgetControl_Handle handle,
                        LV2_HMI_Addressing addressing,
