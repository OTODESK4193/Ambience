...
function (event, funcs) {
    ...
    /* Move the knobs to match the slot the DSP just stepped to.
    ...
    /* The DSP owns the cycle - it decides which slot a press lands on, and
       says so on active_slot. So the knob sync hangs off that rather than off
       the button: watching slot_next would mean reimplementing the skip rule
       here, in a second place, where it could disagree.

       The first report is a baseline, never a recall. active_slot is an output
       port, so its value arrives shortly after load carrying whatever slot the
       restored state named; treating that as a fresh step would push preset
       values over the knob positions the pedalboard just restored. */
    function handleActiveSlot(data, active) {
        showActiveSlot(active);

        var first = data.activeSlot === null || data.activeSlot === undefined;
        var moved = data.activeSlot !== active;

        data.activeSlot = active;

        if (!data.started || first || !moved) {
            return;
        }

        syncKnobs(data, Math.round(data.slotPreset[active] || 0));
    }

    /* Returns the slot number for slotN_preset, else 0.
       "slot1_preset" is slot + digit + _ + suffix, i.e. 6 + suffix.length. */
    ...
    if (event.type === 'start') {
        /* The opening sweep reports every port's current value. Seeding the
           ...
           which one is lit, but recall nothing. */
        ...
        for (var i in ports) {
            ...
            if (sym === 'active_slot') {
                data.activeSlot = Math.round(ports[i].value);
                showActiveSlot(data.activeSlot);
            }
        }
    ...
    if (event.symbol === 'active_slot') {
        handleActiveSlot(data, Math.round(event.value));
        return;
    }
    ...
    /* slot_next is a trigger the user pressed on screen. The DSP acts on it
       and answers on active_slot; there is nothing to do here, and it must
       not reach the algorithm batch as a stray port change. */
    if (event.symbol === 'slot_next') {
        return;
    }
