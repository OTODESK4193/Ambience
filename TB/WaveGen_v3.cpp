...
void processV121_v3(const std::vector<double>& in, std::vector<float>& out) {
    std::vector<DelayLine> fdn(16, DelayLine(8192));
    std::vector<std::vector<DelayLine>> nested(16, std::vector<DelayLine>(3, DelayLine(1024)));
    std::array<double, 16> fb = {0};
    double decayCoeff = std::exp(-6.91 / (3.0 * FS));
    ...
    for(int n=0; n<NUM_SAMPLES; ++n) {
        ...
        // PURE Householder (No sign flipping to prevent DC divergence)
        double sumFb = 0;
        ...
        for(int i=0; i<16; ++i) {
            ...
            d *= decayCoeff;
            fdn[i].write(in[n] + fb[i]); // Full dense input
            fb[i] = d;
            outSum += d;
        }
        out[n] = outSum * 0.125;
...
int main() {
    ...
    processV121_v3(inSine, out); writeRaw("new_Sine.raw", out);
    
    processV120(inSaw, out); writeRaw("old_Saw.raw", out);
    processV121_v3(inSaw, out); writeRaw("new_Saw.raw", out);
    
    processV120(inSquare, out); writeRaw("old_Square.raw", out);
    processV121_v3(inSquare, out); writeRaw("new_Square.raw", out);
    
    processV120(inSync, out); writeRaw("old_Sync.raw", out);
    processV121_v3(inSync, out); writeRaw("new_Sync.raw", out);
    
    processV120(inFM, out); writeRaw("old_FM.raw", out);
    processV121_v3(inFM, out); writeRaw("new_FM.raw", out);
