...
double getEnv(int n) {
    if (n > FS) return 0.0;
    if (n < 480) return (double)n / 480.0; // 10ms attack
    if (n > FS - 480) return (double)(FS - n) / 480.0; // 10ms release
    return 1.0;
}
...
void processV121_v2(const std::vector<double>& in, std::vector<float>& out) {
    std::vector<DelayLine> fdn(16, DelayLine(8192));
    std::vector<std::vector<DelayLine>> nested(16, std::vector<DelayLine>(3, DelayLine(1024)));
    std::array<double, 16> fb = {0};
    double decayCoeff = std::exp(-6.91 / (3.0 * FS));
    ...
    for(int n=0; n<NUM_SAMPLES; ++n) {
        ...
        for (int i = 0; i < 16; ++i) fb[i] = (fb[i] - householderSub) * signFlipping[i];

        double outSum = 0;
        for(int i=0; i<16; ++i) {
            double fracJitter = std::fmod((double)i * 1.6180339887, 1.0) * 11.0; 
            double mainTime = narrowCoprime[i] + fracJitter;
            ...
            for(int s=0; s<3; ++s) {
                ...
                double modTime = fixedLen + modLfo * 8.0; 
...
int main() {
    ...
    for(int n=0; n<NUM_SAMPLES; ++n) {
        ...
        double env = getEnv(n) * 0.5; // Gain staging
    ...
    // Process Sine
    processV120(inSine, out); writeRaw("old_Sine.raw", out);
    processV121_v2(inSine, out); writeRaw("new_Sine.raw", out);
    // Process Saw
    processV120(inSaw, out); writeRaw("old_Saw.raw", out);
    processV121_v2(inSaw, out); writeRaw("new_Saw.raw", out);
    // Process Square
    processV120(inSquare, out); writeRaw("old_Square.raw", out);
    processV121_v2(inSquare, out); writeRaw("new_Square.raw", out);
    // Process Sync
    processV120(inSync, out); writeRaw("old_Sync.raw", out);
    processV121_v2(inSync, out); writeRaw("new_Sync.raw", out);
    // Process FM
    processV120(inFM, out); writeRaw("old_FM.raw", out);
    processV121_v2(inFM, out); writeRaw("new_FM.raw", out);
